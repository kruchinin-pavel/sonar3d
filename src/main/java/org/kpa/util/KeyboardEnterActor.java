...
public class KeyboardEnterActor {
    ...
    private KeyboardEnterActor() {
        File exitFile = new File(Conf.getProperty("exit_on_file", "Exit if file found.", ""));
        logger.info("Keyboard enter listener created. Exit file (will exit on presence of that file): {}", exitFile);
        thr = new Thread(() -> {
            try {
                while (!Thread.currentThread().isInterrupted() && System.in.available() == 0) {
                    Thread.sleep(1000);
                }
                if (System.in.available() > 0) {
                    logger.warn("Symbols are available in System.in.");
                    CHelper.forEachSecure(actors, Runnable::run);
